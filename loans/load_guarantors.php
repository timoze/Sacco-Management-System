...
for ($icount = 0; $icount < $guarantors; $icount++) {
    ...
    <div class="col-md-6 col-sm-6">
        <select name="guarantor[]" id="guarantor'.$icount.'" placeholder="Select Guarantor ..."  class="form-control " required="true">
            ...
            $printout .= '		<option value="'.$row_clients->member_id.'" >'. get_membername_fromid($dbh, $mbrid).'</option>';
																$cnt=$cnt+1;
															}
														} 
                                     $printout .= '		</select>  
    ...
    <div class="col-md-3 col-sm-3">
        <input class="form-control" name="guarantoramount[]" id="guarantoramount'.$icount.'" onBlur="load_repayment_period()" placeholder="Amount" required />
    </div>
</div>';
}
echo $printout;
