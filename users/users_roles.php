  ...
  <body class="nav-md">
          ...
          <div class="col-md-12 col-sm-12 ">
                <div class="x_panel">
                  ...
                  <div class="x_content">
                      ...
                      <a href='javascript: void(0)' onclick=javascript:poptastic('./add_role.php',750,300,'add_role') title='Click to Add New Role' style='text-decoration:none'><button class='btn btn-success'>New Role</button></a>
                      ...
                      <th class="hidden-print"><input type="checkbox" id="check-all"  onClick='this.value=check(this.form);document.form1.go.disabled=false;' ></th>
								...
								<?php
                                $staus = "1";
    $sql="SELECT * from  user_roles WHERE status= :staus order by role_id  ";
    ...
    if ($query->rowCount() > 0) {
        foreach ($results as $row) {
            $role_id    = $row->role_id ;
            $role       = $row->role;
            $edit = '<a id="other" class="dropdown-item" href="javascript: void(0)" onclick=javascript:poptastic("./edit_user_role.php?editid='.$row->role_id.'","750","300","role") title="Click to Edit User Role" style="text-decoration:none">Edit</a>';
            ...
            ?>
                            <tr class="active">
                                ...
                                <td width=80%><?php  echo htmlentities($role);?></td>
                                <td id="other"><?php echo $action;?></td>
                                <td class="hidden-print"><input type="checkbox" id="check-all" class="delete-id" name="chk_id[]" value="<?php echo $row->role_id;?>" onClick="document.go.disabled=false"></td>
                            </tr>
                            <?php $cnt=$cnt+1;
        }
        ?>
        ...
        if (isset($_POST['chk_id'])) {
            ...
            if ($select == '1') {//delete role
                ...
                foreach ($arr as $role_id) {
                    $role_name_from_id = get_role_name($dbh,$role_id);
                    $sql="DELETE from user_roles where role_id=:eid";
                    $query2=$dbh->prepare($sql);
                    $query2->bindParam(':eid', $role_id, PDO::PARAM_STR);
                    $query2->execute();
                    $audit_description = "Deleted User role ID ".$role_id." - (" . $role_name_from_id .")";
